...
int dp[100010][3];
int rec(int level,int color,vector<vector<int>>&arr){
    if(level==arr.size()) return 0;
    
    if(dp[level][color]!=-1) return dp[level][color];
    
    return dp[level][color]=arr[level][color]+min(rec(level+1,(color+1)%3,arr),rec(level+1,(color+2)%3,arr));
}

int Solution::solve(vector<vector<int> > &A) {
    memset(dp,-1,sizeof(dp));
    return min({rec(0,0,A),rec(0,1,A),rec(0,2,A)});
}
